...
for br_int = 1:num_of_br_interval
    ...
    for state = 1:num_of_state
        %if (U_k_p_1(br_int, state) > 1)
            ...
            if(buffer == 1)
                average_underFlow = average_underFlow + sessionProbVector(state)/nInt;
            end
            % average_q = average_q + sessionProbMatrix(br_int, state) * q * Bitrate_Int_Prob(q, br_int); % Working on this and line below
            average_q = average_q + sessionProbVector(state) * q / nInt; % Working on this and line below
            % average_switch = average_switch + sessionProbMatrix(br_int, state) * abs (U_k_p_1(br_int, state) - 1 - q) * Bitrate_Int_Prob(q, br_int);            
            average_switch = average_switch + sessionProbVector(state) * abs (policy(br_int, state) - q) /nInt;            
            average_buffer = average_buffer + sessionProbVector(state) * (buffer) * 2 / nInt;
            %buffer
...
%             end
    ...
    average_buffer_total = average_buffer_total + average_buffer; % / sum_prob_not_stall;
    average_underFlow_total=average_underFlow_total+average_underFlow;
end
...
display(average_underFlow_total);